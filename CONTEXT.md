...
- **Billing page de-clutter** — followed the Option-2 card-with-chip-header restructure. New `src/components/CardBrandIcon.jsx` renders inline SVG logos for Visa / Mastercard / Amex (and Discover gradient placeholder). Payment-method rows show the actual brand icon instead of the generic `CreditCard`. Secondary payment rows, invoice mobile rows, and subscription rows all flattened from per-row bordered cards to **divider-separated list rows** inside the parent card. Billing history header icon swapped `Receipt` → `Clock` to avoid collision with the empty-state `Receipt` icon inside `InvoicesTable`. `mockPaymentMethods` gained a Mastercard entry for icon coverage.
